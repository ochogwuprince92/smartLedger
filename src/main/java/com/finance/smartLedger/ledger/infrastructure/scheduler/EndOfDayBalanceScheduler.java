...
public class EndOfDayBalanceScheduler {
  ...
  public void processEndOfDayBalances() {
    ...
    try {
      Map<String, com.finance.smartLedger.shared.valueobject.Money> trialBalanceDifferences =
          balanceService.calculateTrialBalanceByCurrency();
      boolean isBalanced =
          trialBalanceDifferences.values().stream()
              .allMatch(com.finance.smartLedger.shared.valueobject.Money::isZero);
      ...
      log.info("Trial balance check - Balanced: {}, Differences by currency: {}", isBalanced, trialBalanceDifferences);
      ...
      if (!isBalanced || !accountsWithNegativeBalance.isEmpty()) {
        sendBalanceAlert(isBalanced, trialBalanceDifferences, accountsWithNegativeBalance);
      }
  ...
  private void sendBalanceAlert(boolean isBalanced, Map<String, com.finance.smartLedger.shared.valueobject.Money> trialBalanceDifferences, List<com.finance.smartLedger.ledger.domain.Account> accountsWithNegativeBalance) {
    try {
      ...
      if (!isBalanced) {
        trialBalanceDifferences.forEach(
            (currencyCode, difference) -> {
              if (!difference.isZero()) {
                message.append(String.format("- Trial balance is NOT balanced for %s. Difference: %s\n", currencyCode, difference));
              }
            });
      }
